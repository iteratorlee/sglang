...
class EagleDraftWorker(EagleDraftWorkerBase):
    ...
    def _capture_cuda_graphs(self):
        ...
        if (
            ...
            tic = time.perf_counter()
            before_mem = get_available_gpu_memory(self.device, self.gpu_id)
            extend_capture_bs, _ = get_batch_sizes_to_capture(
                self.draft_runner,
                captured_req_width=self.speculative_num_draft_tokens,
            )
            log_info_on_rank0(
                logger,
                f"Capture draft extend CUDA graph begin. backend={decode_backend}, "
                f"num_tokens_per_req={self.speculative_num_draft_tokens}, "
                f"bs={extend_capture_bs}, avail mem={before_mem:.2f} GB",
            )
    ...
    def draft(self, batch: ScheduleBatch):
        ...
        transaction = getattr(
            ...
            "speculative_state_transaction",
            contextlib.nullcontext,
        )
        with canary_outside_ctx, transaction():
            # Run draft
